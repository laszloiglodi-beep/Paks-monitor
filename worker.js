...
async function fetchVizStation(url, wantExtras = false) {
  ...
  try {
    const response = await fetch(url, {
      headers: {
        "User-Agent": "Mozilla/5.0 (compatible; VPAKS05)"
      },
      cf: {
        cacheTtl: 60,
...
<div class="header">
  ...
  <div class="clockWrap">
    ...
    <span class="refresh">FRISSÍTVE: ${shortTime(oahTime)}</span>
  </div>
    ...
    <div class="pad">
      ...
      <div class="chartPanel">
        ...
        <div class="chartHead">
          ...
          <div class="periods">
            <button class="period" data-hours="6">6 ÓRA</button>
            <button class="period active" data-hours="24">24 ÓRA</button>
  ...
  <div class="panel">
    <div class="pad">
      ...
      <div class="metricsGrid">
        ...
        <div class="metric">
          <div class="metricName">DUNA VÍZHŐ</div>
          <div class="metricValue blue">${tempText}</div>
        </div>

        <div class="metric">
          <div class="metricName">KILÉPŐ VÍZHŐ</div>
          <div class="metricValue orange">—</div>
        </div>
...
<div class="hydroPanel">
  ...
  <div class="scene">
    ...
    <div class="sceneTitle tRiver">DUNA (FŐÁG)</div>
    <div class="sceneTitle tThreshold">FENÉKKÜSZÖB (KŐSZÓRÁS)</div>
    <div class="sceneTitle tHvcs">HIDEGVÍZ-CSATORNA<br>(ÖBLÖZET)</div>
    <div class="sceneTitle tPumps">SZIVATTYÚK<br>(HŰTŐVÍZ)</div>
    <div class="sceneTitle tPlant">PAKSI ATOMERŐMŰ</div>

    <div class="water"></div>
    <div class="bed"></div>

    <div class="reading rRiver">
      <div class="readLabel">AKTUÁLIS</div>
    ...
    <div class="reading rUp">
      <div class="readLabel">FELVÍZ</div>
    ...
    <div class="rackLabel">SZŰRŐRÁCS</div>
    <div class="rack"></div>
    ...
    <div class="plant">
      ...
      <div class="plantName">PAKSI<br>ATOMERŐMŰ</div>
    ...
    <div class="flowArrow fa1">→</div>
    <div class="flowArrow fa2">→</div>
    <div class="flowArrow fa3">↑</div>
    <div class="flowArrow fa4">→</div>
  ...
  <div class="bottomRail">
    ...
    <div class="bottomCard">
      <div class="bottomLabel">DUNA – PAKS (FŐÁG)</div>
      <div class="bottomValue blue">
        ${Number.isFinite(water) ? water + " cm" : "—"}
      </div>
      <div class="bottomSub">
        ${Number.isFinite(riverMbf) ? fmt2(riverMbf) + " mBf" : "—"}
      </div>
      <div class="bottomSub">🌊 ${flowText}</div>
    </div>

    <div class="bottomCard">
      <div class="bottomLabel">FENÉKKÜSZÖB FELVÍZ</div>
      <div class="bottomValue blue">
        ${Number.isFinite(thresholdUp) ? thresholdUp + " cm" : "—"}
      </div>
      <div class="bottomSub">
        ${Number.isFinite(thresholdUpMbf) ? fmt2(thresholdUpMbf) + " mBf" : "—"}
      </div>
    </div>

    <div class="bottomCard">
      <div class="bottomLabel">FENÉKKÜSZÖB ALVÍZ</div>
      <div class="bottomValue blue">
        ${Number.isFinite(thresholdDown) ? thresholdDown + " cm" : "—"}
      </div>
      <div class="bottomSub">
        ${Number.isFinite(thresholdDownMbf) ? fmt2(thresholdDownMbf) + " mBf" : "—"}
      </div>
    </div>

    <div class="bottomCard highlight">
      <div class="bottomLabel">DUZZASZTÁS EREDMÉNYE</div>
      ...
      <div class="bottomSub">FELVÍZ − ALVÍZ</div>
    </div>

    <div class="bottomCard">
      <div class="bottomLabel">HIDEGVÍZ-CSATORNA</div>
      <div class="bottomValue blue">
        ${Number.isFinite(hvcs) ? hvcs + " cm" : "—"}
      </div>
      <div class="bottomSub">
        ${Number.isFinite(hvcsMbf) ? fmt2(hvcsMbf) + " mBf" : "—"}
      </div>
    </div>

    <div class="bottomCard">
      <div class="bottomLabel">SZIVATTYÚ SZINT (ÖBLÖZETBEN)</div>
      <div class="bottomValue blue">
        ${Number.isFinite(hvcsMbf) ? fmt2(hvcsMbf) + " mBf" : "—"}
      </div>
      <div class="bottomSub">
        ${Number.isFinite(safetyDistance) ? "TARTALÉK: " + safetyDistance + " cm" : "—"}
      </div>
    </div>

    <div class="bottomCard">
      <div class="bottomLabel">ATOMERŐMŰ TELJESÍTMÉNYE</div>
...
<div class="footer">
  <div>ADATFORRÁSOK: OAH • OVF / VÍZÜGY</div>
  <div>AZ ADATOK TÁJÉKOZTATÓ JELLEGŰEK.</div>
  <div>${VERSION}</div>
</div>
...
<div class="toast" id="toast">Link másolva</div>
...
async function drawPowerChart(hours) {
  ...
  if (data.length === 0) {
    ...
    ctx.textAlign = "left";
    ctx.fillText("Új mérésre várunk…",pad.left+6,H/2);
    return;
  }
...
document
  ...
  ?.addEventListener("click",async () => {
    try {
      ...
      window.prompt("Másold a linket:",PUBLIC_URL);
    }
  });
...
/* Nincs visualViewport resize listener: pinch-zoom alatt a teljes dashboard együtt marad. */
</script>
